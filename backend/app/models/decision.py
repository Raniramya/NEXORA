...
class IntegratedDecisionWorkflow(Base):
 __tablename__="integrated_decision_workflows"; id:Mapped[str]=mapped_column(String(36),primary_key=True,default=lambda:str(uuid.uuid4())); decision_id:Mapped[str]=mapped_column(String(36),ForeignKey("decisions.id"),unique=True,index=True); maintenance_plan_id:Mapped[str]=mapped_column(String(36),ForeignKey("maintenance_plans.id"),unique=True,index=True); readiness_status:Mapped[str]=mapped_column(String(32)); evidence_graph:Mapped[dict]=mapped_column(JSON); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now())


class DecisionReview(Base):
 __tablename__="decision_reviews"; id:Mapped[str]=mapped_column(String(36),primary_key=True,default=lambda:str(uuid.uuid4())); decision_id:Mapped[str]=mapped_column(String(36),ForeignKey("decisions.id"),index=True); reviewer:Mapped[str]=mapped_column(String(120)); outcome:Mapped[str]=mapped_column(String(32)); notes:Mapped[str|None]=mapped_column(String); created_action_ids:Mapped[list]=mapped_column(JSON,default=list); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now())
